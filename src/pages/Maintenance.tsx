...
import { SearchIcon, FilterIcon, PlusIcon, EyeIcon, PencilIcon, Trash2Icon, CheckCircle2Icon, AlertCircleIcon, ClockIcon, WrenchIcon, User2Icon, HomeIcon, LayersIcon, ArrowLeftIcon, CalendarIcon, XIcon } from 'lucide-react';
...
// Add mockUnits for property/unit selection
const mockUnits = [
  { id: 1, property: 'Westlands Apartment', number: 'G1', tenant: { id: 1, name: 'John Mwangi' } },
  { id: 2, property: 'Westlands Apartment', number: 'G2', tenant: null },
  { id: 3, property: 'Westlands Apartment', number: '1A', tenant: null },
  { id: 4, property: 'Westlands Apartment', number: '1B', tenant: null },
  { id: 5, property: 'Kilimani Townhouse', number: 'G1', tenant: { id: 2, name: 'Sarah Ochieng' } },
];
...
export const Maintenance = () => {
  ...
  const selectedUnit = mockUnits.find(unit => unit.id === selectedUnitId);
    ...
    },
    {
      ...
      cell: info => (
        <div className="flex gap-2">
          ...
          <button className="p-2 rounded-lg hover:bg-yellow-100 transition-colors flex items-center gap-1 text-yellow-700 font-semibold shadow-sm" title="Assign" onClick={e => { e.stopPropagation(); /* setAssignRequest(info.row.original); */ }}>
            <LayersIcon size={16} /> Assign
          </button>
        ...
        {editRequest && (
          <form className="flex flex-col gap-4">
            ...
            <div>
              ...
              <select id="editPriority" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
            ...
            <div>
              ...
              <select id="editStatus" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
        ...
        {deleteRequest && (
          <div className="text-center">
            ...
            <div className="flex justify-center gap-4">
              ...
              <button onClick={() => { setDeleteRequest(null); }} className="px-4 py-2 bg-red-700 rounded-lg text-white text-sm hover:bg-red-800">Delete</button>
            </div>
          </div>
        )}
      </Modal>
      ...
      >
        <form className="flex flex-col gap-4">
          <div>
            ...
            <input type="text" id="newTitle" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" />
          </div>
          <div>
            <label htmlFor="newDescription" className="block text-sm font-medium text-gray-700 mb-1">Description</label>
            <textarea id="newDescription" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" rows={4} />
          </div>
          <div>
            ...
            <Listbox value={selectedUnitId} onChange={setSelectedUnitId}>
              <div className="relative mt-1">
                <Listbox.Button className="w-full border rounded-lg px-3 py-2 text-left">
                  {selectedUnit ? `${selectedUnit.property} ${selectedUnit.number}` : <span className="text-gray-400">Select property/unit...</span>}
                </Listbox.Button>
                <Listbox.Options className="absolute z-10 mt-1 w-full bg-white border border-green-100 rounded-lg shadow-lg max-h-60 overflow-auto focus:outline-none">
                  {mockUnits.map(unit => (
                    <Listbox.Option key={unit.id} value={unit.id} className={({ active, selected }) => `px-4 py-2 cursor-pointer ${active ? 'bg-green-100' : ''} ${selected ? 'font-bold' : ''}`}>{unit.property} {unit.number}</Listbox.Option>
                  ))}
                </Listbox.Options>
              </div>
            </Listbox>
          ...
          {selectedUnit && selectedUnit.tenant && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Tenant</label>
              <input className="w-full px-3 py-2 border border-gray-100 rounded-lg bg-gray-100 text-gray-700" value={selectedUnit.tenant.name} readOnly />
            </div>
          )}
          <div>
            <label htmlFor="newDateReported" className="block text-sm font-medium text-gray-700 mb-1">Date Reported</label>
            <input type="date" id="newDateReported" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" />
          </div>
          <div>
            <label htmlFor="newPriority" className="block text-sm font-medium text-gray-700 mb-1">Priority</label>
            <select id="newPriority" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
              <option value="high">High</option>
              <option value="medium">Medium</option>
              <option value="low">Low</option>
            </select>
          ...
          <div>
            ...
            <select id="newStatus" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
              <option value="pending">Pending</option>
              <option value="in_progress">In Progress</option>
              <option value="completed">Completed</option>
            </select>
